...
const getPrices = (document, proxy) => {
    ...
    if(document?.pricing?.contentAccess === 'free') {
        response.free = true
    } else {
        response.plan = getPlan(document, proxy)
        response.buy = getBuyPrice(document, proxy)
        response.rent = getRentPrice(document, proxy)
    }

    if(response.plan || response.buy || response.rent || response.free) {
